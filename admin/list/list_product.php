    ...
    <div class="col-sm-12">
      <div class="card-box table-responsive">
        <div class="btn__add" style = " margin-top: 15px; border-radius:3px">
          <a href="./dashboard.php?page=add/add_product" style = "padding: 9px 28px; border: 1px solid;border-radius:3px"><i class="fa fa-plus"></i>Thêm sản phẩm</a>
        </div>
        ...
        <tbody>
          <?php
            $qry = $conn->query("select masp,tensp,LoaiSP.tenloai as tenloais,soluong,gia,anh,Sanpham.mota as motas from Sanpham,LoaiSP where Sanpham.maloai = LoaiSP.maloai ORDER BY masp DESC");
            while($row= $qry->fetch_assoc()):
          ?>
          <tr>
            <td><b>SP-<?php echo $row['masp'] ?></b></td>
            <td><b><?php echo ($row['tensp']) ?></b></td>
            <td><b><?php echo ($row['tenloais'])?></b></td>
            <td><b><?php echo $row['soluong'] ?></b></td>
            <td><b><?php echo number_format($row['gia'],0,',','.').' đ' ?></b></td>
            <td class="text-center">
              <div class="btn-group">
                  <form action="./dashboard.php?page=update/update_product" method = "POST">
                ...
                <a href="./dashboard.php?page=view_product&masp=<?php echo $row['masp'] ?>" class="btn btn-info btn-flat">
                  <i class="fas fa-eye"></i>
                </a>
                <a href="./dashboard.php?page=delete/delete_product&masp=<?php echo $row['masp'] ?>" class="btn-infos btn  btn-flat">
